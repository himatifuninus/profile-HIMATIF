...
const Register = () => {
  ...
  const handleTeamNameChange = (e) => {
    ...
    setErrors((prev) => ({
      ...prev,
      teamName: found ? `Kata "${found}" tidak diperbolehkan` : "",
    }));
    setTeamName(value);
  };

  const handleMemberChange = (index, field, value) => {
    const found = field === "member_name" ? containsForbiddenWord(value) : null;
    const newMemberErrors = [...errors.members];
    newMemberErrors[index] = found ? `Kata "${found}" tidak diperbolehkan` : "";
    setErrors((prev) => ({ ...prev, members: newMemberErrors }));
  ...
  const hasErrors = () =>
    !!errors.teamName || errors.members.some((e) => e !== "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ teamName: "", members: Array(5).fill("") });
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="relative mb-10">
            <input
              ...
              required
              placeholder=" "
              className="peer w-full bg-transparent border-b-2 border-gray-400 text-white py-2 pr-8 outline-none focus:border-blue-500"
            />
            <label className="absolute left-0 top-2 text-gray-400 transition-all duration-200
              peer-placeholder-shown:top-2 peer-placeholder-shown:text-sm
              peer-focus:-top-4 peer-focus:text-xs peer-focus:text-blue-400
              peer-not-placeholder-shown:-top-4 peer-not-placeholder-shown:text-xs">
              Nama Tim
            </label>
            <span className="absolute right-2 top-2 text-gray-400"><FaUser /></span>
            {errors.teamName && (
          ...
          <div className="relative mb-10">
            ...
            <span className="absolute right-2 top-2 text-gray-400"><FaPhone /></span>
          </div>

          {/* 5 Anggota */}
          {members.map((member, index) => (
            <div key={index} className="mb-6 border border-slate-600 rounded p-4">
              ...
              <div className="relative mb-8">
                ...
                {errors.members[index] && (
                  <p className="text-red-500 text-xs mt-1">{errors.members[index]}</p>
                )}
              </div>
              ...
              <div className="relative mb-2">
                ...
                <span className="absolute right-2 top-2 text-gray-400"><FaIdCard /></span>
              </div>
            </div>
          ))}
